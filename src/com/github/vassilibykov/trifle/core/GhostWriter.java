...
public class GhostWriter {
    ...
    /**
     * Generate a code snippet which unwraps a Boolean on the stack, or does
     * whatever is generated by the parameter if the value on the stack is not
     * a Boolean.
     *
     * @param failureCodeGenerator A procedure running which generates the handler
     *        for the failure case. The generated code must throw an exception at the end.
     */
    public GhostWriter unwrapBooleanOr(Runnable failureCodeGenerator) {
        dup();
        instanceOf(BOOLEAN_ICN);
        withLabelAtEnd(onSuccess -> {
            jumpIfNot0(onSuccess);
            failureCodeGenerator.run();
        });
    ...
    /**
     * Similar to {@link #unwrapBooleanOr(Runnable)}.
     */
